...
export class BacktestEngine {
    ...
    private async fetchHistoricalData(symbol: string, startDate: Date, endDate: Date): Promise<Candle[]> {
        ...
        const oiLimit = 500;
        const timeframeMs = timeframe === '15m' ? 900000 : 300000;
        ...
        while (currentTime < endTime) {
            ...
            while (oiCursor <= chunkEndTime) {
                ...
                const oiHistory = await this.binance.getHistoricalOpenInterest(symbol, timeframe, oiLimit, oiCursor, requestEndTime);
                if (oiHistory.length === 0) break;

                oiHistory.forEach(item => {
                    oiDataMap.set(Number(item.timestamp), parseFloat(item.sumOpenInterest));
                });
            ...
            const mergedCandles = candles.map(c => {
                ...
                return { ...c, openInterest: lastKnownOI, takerBuyBaseVolume: c.takerBuyBaseVolume || 0 };
            });
        ...
        if (sorted.length > 0) db.saveCandles(symbol, timeframe, sorted);
        return sorted;
    }
